...
#define RING_INFO(fmt,arg...) \
	log_print(LOG_TAG,LOG_LEVEL_INFO,fmt,##arg)

#define RING_DEBUG(fmt,arg...) \
	log_print(LOG_TAG,LOG_LEVEL_DEBUG,fmt,##arg)

#define RING_ERROR(fmt,arg...) \
	log_print(LOG_TAG,LOG_LEVEL_ERROR,fmt,##arg)


#define RING_DUMP(fmt,arg...) \
	log_print(LOG_TAG,LOG_LEVEL_MSGDUMP,fmt,##arg)

#define RING_WARN(fmt,arg...) \
	log_print(LOG_TAG,LOG_LEVEL_WARNING,fmt,##arg)

#define RING_EXCESSIVE(fmt,arg...) \
	log_print(LOG_TAG,LOG_LEVEL_EXCESSIVE,fmt,##arg)
...
{
    RING_EXCESSIVE("ring buffer start enter\n");
    pthread_mutex_lock(&q->lock);
    if (q->enable == true)
        goto end;
    ...
    pthread_mutex_unlock(&q->lock);
    RING_EXCESSIVE("ring buffer start quit\n");
    return 0;
}
