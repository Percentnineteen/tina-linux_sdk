...
static void LCD_panel_init(u32 sel)
{
	...
	st7701s_3spi_spi_write_cmd(0x01);
	...
	st7701s_3spi_spi_write_data(0x08); //18-bit/pixel: MDT=0:D[21:16]=R,D[13:8]=G,D[5:0]=B(CDH=00) ;
	...
	st7701s_3spi_spi_write_data(0x66); //55/50=16bit(RGB565);66=18bit(RGB666);77或默认不写3AH是=24bit(RGB888)

	st7701s_3spi_spi_write_cmd(0x36);
	st7701s_3spi_spi_write_data(0x00);

	st7701s_3spi_spi_write_cmd(0x21);  // Contrast

	st7701s_3spi_spi_write_cmd(0x11);
	sunxi_lcd_delay_ms(140);
...
__lcd_panel_t st7701s_3spi_panel = {
