...
static int sunxi_spi_mode_check(void __iomem  *base_addr, u32 tcnt, u32 rcnt, u32 cmdcnt, u8 cmd)
{
	...
	if (SPINOR_OP_READ_1_1_4 == cmd || SPINOR_OP_READ_1_1_4_4B == cmd ||
		...
		if (!cmdcnt && tcnt && rcnt) {
			...
			spi_set_bc_tc_stc(max(tcnt, rcnt), 0, max(tcnt, rcnt), 0, base_addr);
		} else {
...
int spi_gain_config_clk(struct sunxi_spi_slave *sunxi_slave)
{
	int ret = 0, nodeoffset = 0;
	u32 rval = 0;
	char spi_board[20];

	sprintf(spi_board, "spi%d", sunxi_slave->bus);
	nodeoffset =  fdt_path_offset(working_fdt, spi_board);
	if (nodeoffset < 0) {
		SPI_INF("get spi%d para fail\n", sunxi_slave->bus);
	} else {
		ret = fdt_getprop_u32(working_fdt, nodeoffset,
				"clock-frequency", (uint32_t *)(&rval));
		if (ret < 0)
	...
	SPI_INF("spi sunxi_slave->max_hz:%d\n", sunxi_slave->max_hz);
				  ...
				  unsigned int max_hz, unsigned int mode)
{
	...
	spi_gain_config_clk(sunxi_slave);
	...
	/* Usually spi0 is used for Nor/Nand Flash didn't use the default delay sample */
	if (sunxi_slave->bus != CONFIG_SF_DEFAULT_BUS) {
		spi_config_tc(sunxi_slave, 1, SPI_MODE_3,
			(void __iomem *)(unsigned long)sunxi_slave->base_addr);
	}
